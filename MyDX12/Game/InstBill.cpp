...
void InstBill::Update()
{
	...
	if (SUCCEEDED(result))
	{
		constMap->matbillboard = d_camera->GetMyBillboard();
		constMap->viewproj = d_camera->GetMyViewProjection();
		for (int i = 0; i < dataIndexCount; i++)
		{
			// ワールド行列の更新
			Math::Matrix4 matScale, matTrans;
			this->myWorld.Identity();
			Math::Vector4 colorMap = { 0,0,0,0 };

			if (iData.size() >= i + 1)
			{
				matScale = Math::Scaling(iData[i].scale, iData[i].scale, iData[i].scale);
				matTrans = Math::Translation(iData[i].position.x, iData[i].position.y, iData[i].position.z);
				this->myWorld *= matScale;
				this->myWorld *= matTrans;
			...
			constMap->data[i].mat = this->myWorld;
...
void InstBill::DrawBillBox(Math::Vector3 position, float scale, float r, float g, float b, float a)
{
...
void InstBill::ReLoadTexture(const std::string& textureFilename)
{
	// もう一度読み込む

...
Math::Matrix4 Circle::myProjection;
	...
	if (SUCCEEDED(result))
	{
		for (int i = 0; i < dataIndexCount; i++)
		{
			...
			if (datas.size() >= i + 1)
			{
				...
				myScale = Math::Scaling(datas[i].radian, datas[i].radian, 0.0f);
				matTrans = XMMatrixTranslation(datas[i].position.x, datas[i].position.y, 0.0f);
				myTrans = Math::Translation(datas[i].position.x, datas[i].position.y, 0.0f);
				this->myWorld *= myScale;
				this->myWorld *= myTrans;
			...
			constMap->data[i].myMat = this->myWorld * myProjection;

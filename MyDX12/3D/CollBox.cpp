...
void CollisionBox::Update()
{
	...
	Math::Matrix4 myScl,myRot,myTrans;
	myScl = Math::Scaling(scale.x, scale.y, scale.z);
	myRot.Identity();
	myRot *= Math::RotZ(Math::ConvertToRadians(rotation.z));
	myRot *= Math::RotX(Math::ConvertToRadians(rotation.x));
	myRot *= Math::RotY(Math::ConvertToRadians(rotation.y));
	myTrans = Math::Translation(position.x, position.y, position.z);

	myWorld.Identity(); // 変形をリセット
	myWorld *= myScl;
	myWorld *= myRot; // ワールド行列に回転を反映

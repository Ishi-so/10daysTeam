...
void Tessellation::CreateTexture()
{
	...
	texBuff0 = LoadTextureFromFile(texHeightPath);
	texBuff1 = LoadTextureFromFile(texNormalPath);
}
...
void Tessellation::CreateSRV()
{
	...
	srvDesc.Format = texBuff0->GetDesc().Format;
	// デスクリプタヒープにRTV作成
	device->CreateShaderResourceView(texBuff0.Get(),
		&srvDesc,
	...
	srvDesc.Format = texBuff0->GetDesc().Format;
	// デスクリプタヒープにRTV作成
	device->CreateShaderResourceView(texBuff1.Get(),
		&srvDesc,
		CD3DX12_CPU_DESCRIPTOR_HANDLE(
...
void Tessellation::Update()
{
	...
	Math::Matrix4 myTrans;
	myTrans = Math::Translation(position.x, position.y, position.z);

	myWorld.Identity(); // 変形をリセット
	myWorld *= myTrans; // ワールド行列に平行移動を反映

	const Math::Matrix4& myViewProjection = d_camera->GetMyViewProjection();
	const Math::Vector3& cameraPos = { d_camera->GetPos_X(),d_camera->GetPos_Y(),d_camera->GetPos_Z() };
